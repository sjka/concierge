...
public class BundlesWithExplodedJarContentTest extends
	...
	@Test
	public void testBundleWithExplodedJarContentListResources() throws Exception {
		...
		ClassLoader classLoader = bundleUnderTest.adapt(BundleWiring.class).getClassLoader();
		...
		Collection<String> resourceList = bundleUnderTest.adapt(BundleWiring.class).listResources("/" + uri.getFragment() + "/", "*.txt", 0);
		assertNotNull(resourceList);
		assertEquals(1, resourceList.size());
		assertEquals("res/somefile.txt", resourceList.iterator().next());
	}
